...
if PROMO_MAX_USES == 0:
    PROMO_MAX_USES = None  # 0 = unlimited
...
def log_event(
    ...
    try:
        ...
        logger.exception("Could not save DB log event=%s", event)


def increment_stat(metric: str, amount: Decimal | int | float = 1):
    ...
    try:
        ...
        logger.exception("Could not update daily stat=%s", metric)
...
def init_db():
    ...
    with db() as conn:
        ...
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_hugs_user_id ON hugs(user_id)"
        )
        conn.execute(
        ...
        now = utcnow()
        conn.execute(
            "UPDATE profiles SET first_seen_at=%s WHERE first_seen_at IS NULL",
            (now,),
        )

        # Keep the environment promo available without overwriting an admin-created code.
        if PROMO_CODE:
            promo_plan = PROMO_PLAN if PROMO_PLAN in PLANS else "month"
            conn.execute(
                """
                INSERT INTO promo_codes(code, plan, max_uses, used, active, expires_at)
                VALUES (%s, %s, %s, 0, TRUE, NULL)
                ON CONFLICT(code) DO NOTHING
                """,
                (PROMO_CODE, promo_plan, PROMO_MAX_USES),
            )
            # Repair an old version where MAX_USES=0 accidentally meant exhausted.
...
def ensure_profile(user_id: int, telegram_user=None) -> bool:
    now = utcnow()
    ...
    with db() as conn:
        inserted = conn.execute(
            ...
            INSERT INTO profiles(
                ...
                level, warmth, ref_code, checks, first_seen_at, referral_bonus_days
            )
            VALUES(%s,%s,%s,%s,0,1000,'HUGGER',0,%s,0)
    ...
    if inserted:
        ...
        log_event("INFO", "user_registered", user_id)
        return True
    return False


def get_profile(user_id: int, telegram_user=None) -> dict:
    ensure_profile(user_id, telegram_user)
    with db() as conn:
        row = conn.execute(
        ...
        sent = conn.execute(
            "SELECT COALESCE(SUM(count),0) AS total FROM hugs WHERE user_id=%s",
            (user_id,),
        ).fetchone()
        searches = conn.execute(
            "SELECT COUNT(*) AS total FROM searches WHERE user_id=%s",
            (user_id,),
        ).fetchone()
    profile = dict(row)
    profile["sent"] = int(sent["total"] or 0)
    profile["searches"] = int(searches["total"] or 0)
    subscription = get_active_subscription(user_id)
    profile["sub"] = subscription
    profile["sub_active"] = subscription is not None
    profile["sub_days_left"] = 0
    if subscription:
        expires = aware(subscription["expires_at"])
        profile["sub_days_left"] = max(
            0,
            int(((expires - utcnow()).total_seconds()) // 86400),
        )
    return profile
...
def activate_subscription(
    ...
    if payment_id:
        ...
        if existing:
            return aware(existing["expires_at"]), False
    ...
    with db() as conn:
        ...
        bonus_row = conn.execute(
            "SELECT referral_bonus_days FROM profiles WHERE user_id=%s FOR UPDATE",
            (user_id,),
        ).fetchone()
        referral_bonus_days = int(
            (bonus_row["referral_bonus_days"] if bonus_row else 0) or 0
        )
        starts_at = max(now, aware(current["expires_at"]) if current else now)
        expires_at = starts_at + timedelta(days=PLANS[plan]["days"])
        if consume_referral_bonus and referral_bonus_days > 0:
            expires_at += timedelta(days=referral_bonus_days)
        try:
            row = conn.execute(
                """
            ...
            if not row and payment_id:
                existing = conn.execute(
                    "SELECT expires_at FROM subscriptions WHERE payment_id=%s LIMIT 1",
                    (payment_id,),
                ).fetchone()
                conn.commit()
                return aware(existing["expires_at"]), False
            if consume_referral_bonus and referral_bonus_days > 0:
                conn.execute(
                    "UPDATE profiles SET referral_bonus_days=0 WHERE user_id=%s",
                    (user_id,),
                )
            conn.commit()
    ...
    log_event(
        ...
        user_id,
        {"plan": plan, "method": method, "payment_id": payment_id},
    )
...
def create_payment(
    ...
    with db() as conn:
        if external_id:
            existing = conn.execute(
                "SELECT id FROM payments WHERE external_id=%s LIMIT 1",
                (external_id,),
            ).fetchone()
            if existing:
        ...
        row = conn.execute(
            ...
            INSERT INTO payments(
                user_id, plan, method, external_id,
...
def request_usage(user_id: int) -> tuple[bool, int]:
    ...
    with db() as conn:
        row = conn.execute(
            """
            SELECT requests FROM daily_usage
        ...
        if not row:
            ...
            conn.execute(
                """
                INSERT INTO daily_usage(user_id, usage_date, requests)
                VALUES(%s,%s,1)
                """,
                (user_id, today),
            )
        else:
            current = int(row["requests"])
            ...
            conn.execute(
                """
                UPDATE daily_usage SET requests=%s
                WHERE user_id=%s AND usage_date=%s
                """,
                (used, user_id, today),
            )
...
def usage_today(user_id: int) -> int:
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT requests FROM daily_usage WHERE user_id=%s AND usage_date=%s",
            (user_id, today),
        ).fetchone()
...
def add_hug(user_id: int, target: str, count: int):
    ...
    with db() as conn:
        conn.execute(
            "INSERT INTO hugs(user_id,target,count,created_at) VALUES(%s,%s,%s,%s)",
        ...
        conn.execute(
            ...
            SET warmth=LEAST(1000,warmth+10),
                level=GREATEST(level, 1 + ((SELECT COUNT(*) FROM hugs WHERE user_id=%s)::INT / 25))
            WHERE user_id=%s
            """,
            (user_id, user_id),
        )
        conn.commit()
    increment_stat("hugs", count)
    log_event("INFO", "hug_completed", user_id, {"target": target, "count": count})


def add_check(user_id: int):
    with db() as conn:
        conn.execute(
            "UPDATE profiles SET checks=checks+1 WHERE user_id=%s",
            (user_id,),
        )
        conn.commit()
    increment_stat("checks")


def add_search(user_id: int, query: str):
    with db() as conn:
        conn.execute(
            "INSERT INTO searches(user_id,query,created_at) VALUES(%s,%s,%s)",
            (user_id, query, utcnow()),
        )
        conn.commit()
    increment_stat("searches")
    log_event("INFO", "search_completed", user_id, {"query": query})
...
def add_referral(referrer_id: int, referred_id: int) -> bool:
    if referrer_id == referred_id:
        return False
    ensure_profile(referrer_id)
    ensure_profile(referred_id)
    with db() as conn:
        row = conn.execute(
            """
            ...
            RETURNING id
            """,
            (referrer_id, referred_id, utcnow()),
        ).fetchone()
        conn.commit()
    if row:
        increment_stat("referrals")
        log_event(
            "INFO",
            "referral_added",
            referred_id,
...
def get_referral_rewarded_count(user_id: int) -> int:
    with db() as conn:
        row = conn.execute(
            "SELECT COUNT(*) AS total FROM referrals WHERE referrer_id=%s AND rewarded=TRUE",
            (user_id,),
        ).fetchone()
    return int(row["total"])


def get_referral_bonus_days(user_id: int) -> int:
    with db() as conn:
        row = conn.execute(
            "SELECT referral_bonus_days FROM profiles WHERE user_id=%s",
            (user_id,),
        ).fetchone()
    return int((row["referral_bonus_days"] if row else 0) or 0)
...
async def reward_referrer_for_purchase(bot, referred_user_id: int):
    with db() as conn:
        ...
        updated = conn.execute(
            """
            UPDATE referrals
            SET rewarded=TRUE, rewarded_at=%s
            WHERE id=%s AND rewarded=FALSE
            """,
            (utcnow(), referral["id"]),
        )
        ...
        extra_tier_days = 0
        awarded_tiers: list[tuple[int, int]] = []
        for threshold, days in REFERRAL_TIERS:
            if rewarded_count >= threshold:
                tier = conn.execute(
                    """
                    INSERT INTO referral_tier_rewards(referrer_id,threshold,days,created_at)
                    VALUES(%s,%s,%s,%s)
                    ON CONFLICT(referrer_id,threshold) DO NOTHING
                    RETURNING id
                    """,
                    (referrer_id, threshold, days, utcnow()),
                ).fetchone()
                if tier:
                    extra_tier_days += days
                    awarded_tiers.append((threshold, days))

        total_days = REFERRAL_REWARD_DAYS + extra_tier_days
        ...
        if subscription:
            new_expiry = aware(subscription["expires_at"]) + timedelta(days=total_days)
            conn.execute(
            ...
            conn.execute(
                """
                UPDATE profiles
                SET referral_bonus_days=referral_bonus_days+%s
                WHERE user_id=%s
                """,
                (total_days, referrer_id),
            )

        conn.commit()

    if extra_tier_days:
        increment_stat("tier_rewards", extra_tier_days)
    log_event(
        "INFO",
        "referral_rewarded",
        referrer_id,
    ...
    try:
        await bot.send_message(
            ...
            (
                ...
                f"Вам начислено +{REFERRAL_REWARD_DAYS} день подписки."
                f"{tier_text}"
            ),
        )
...
def redeem_promo(user_id: int, raw_code: str) -> tuple[str, str | None]:
    ...
    with db() as conn:
        row = conn.execute(
            "SELECT * FROM promo_codes WHERE code=%s FOR UPDATE",
            (code,),
        ).fetchone()
        ...
        already = conn.execute(
            "SELECT 1 FROM promo_redemptions WHERE code=%s AND user_id=%s",
            (code, user_id),
        ).fetchone()
        if already:
            conn.commit()
        ...
        conn.execute(
            "UPDATE promo_codes SET used=used+1 WHERE code=%s",
            (code,),
        )
        conn.commit()
    return "ok", row["plan"]
...
async def grant_promo_access(bot, user_id: int, plan: str, code: str):
    ...
    existing = find_subscription_by_payment(payment_key)
    if existing:
        expires = aware(existing["expires_at"])
    else:
...
def deactivate_promo(code: str):
    with db() as conn:
        conn.execute(
            "UPDATE promo_codes SET active=FALSE WHERE code=%s",
            (code.upper(),),
        )
        conn.commit()
...
async def apply_promo_text(
    ...
    if status != "ok":
        await update.message.reply_text(
            messages.get(status, "❌ Не удалось применить промокод."),
...
def kb_menu():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("sn1cти аккаунт❄️", callback_data="menu:hug")],
        [
            InlineKeyboardButton("👀 П0иск", callback_data="menu:search"),
            InlineKeyboardButton("🔎 Проверить", callback_data="menu:check"),
        ],
        [InlineKeyboardButton("📜 История поиска", callback_data="menu:history")],
        [InlineKeyboardButton("🏠 На главную", callback_data="nav:home")],
    ])
...
def kb_confirm_hug():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Да sn1cти аккаунт!", callback_data="hug:yes")],
...
def kb_hooray():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Ура! 🎉", callback_data="nav:home")]
    ])


def kb_sub_plans():
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Неделя — 200⭐", callback_data="sub:week")],
        [InlineKeyboardButton("Месяц — 350⭐", callback_data="sub:month")],
        [InlineKeyboardButton("Год — 500⭐", callback_data="sub:year")],
...
def kb_admin_promo_list(rows):
    ...
    buttons.extend([
        [InlineKeyboardButton("➕ Создать / изменить", callback_data="admin:promo_create")],
...
def required_channel_url() -> str:
    ...
    channel = REQUIRED_CHANNEL_ID.strip()
    if not channel:
        return ""
...
async def is_required_channel_member(bot, user_id: int) -> bool:
    ...
    try:
        ...
        logger.error(
            "Required channel check failed: channel=%s user=%s error=%s",
            REQUIRED_CHANNEL_ID,
            user_id,
            exc,
        )
...
async def channel_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not REQUIRED_CHANNEL_ID or not user or user.is_bot or is_admin(user.id):
        return

    q = update.callback_query
    if q and q.data == "gate:check":
        subscribed = await is_required_channel_member(context.bot, user.id)
        if subscribed:
            try:
...
async def show_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await send_ui(update, context, GREETING, kb_home(user_id))


async def show_profile(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    profile = get_profile(user_id, update.effective_user)
    await send_ui(
        update,
        context,
        profile_caption(profile, user_id),
        kb_profile(),
...
def profile_caption(profile: dict, user_id: int) -> str:
    sub_text = "✅ Активна" if profile["sub_active"] else "❌ Не оформлена"
    expires_text = ""
    if profile["sub"]:
        expires = aware(profile["sub"]["expires_at"])
        expires_text = f"\n⏳ До: {expires.strftime('%d.%m.%Y %H:%M')}"
    bonus_days = get_referral_bonus_days(user_id)
    bonus_text = f"\n🎁 Реферальный бонус — +{bonus_days} д." if bonus_days > 0 else ""
    return (
        "👤 Личный кабинет\n\n"
        ...
        f"💎 Подписка — {sub_text}{expires_text}{bonus_text}\n"
        f"📊 Запросов сегодня — {usage_today(user_id)}/{DAILY_REQUEST_LIMIT}"
    )
...
async def run_hug_animation(
    ...
    try:
        ...
        for percent in [8, 20, 34, 49, 68, 71, 90]:
            await bot.send_message(chat_id=chat_id, text=f"💤{percent}%💤")
            await asyncio.sleep(1)
        await bot.send_message(chat_id=chat_id, text="💤100%💤")
        ...
        log_event("ERROR", "hug_animation_failed", user_id, {"target": target})
...
async def confirm_and_send_hug(
    ...
    context.user_data["state"] = None
    target = context.user_data.pop("hug_target", "другу")
    initial_text = (
...
async def do_check(
    ...
    await update.message.reply_text(
        ...
        f"🤗 Обнимашковость — {random.randint(60,100)}%\n\n"
        "✅ Проверка завершена.",
        reply_markup=kb_menu(),
    )
...
async def do_search(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    await send_ui(update, context, "Введите запрос для поиска:", kb_menu())
...
async def show_history(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    ...
    if not history:
        ...
        return
    lines = [
        f"• {row['query']} — {aware(row['created_at']).strftime('%d.%m.%Y %H:%M')}"
        for row in history
    ]
    await send_ui(update, context, "📜 История п0иска:\n\n" + "\n".join(lines), kb_menu())
...
async def show_referrals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    user_id = update.effective_user.id
    link = await get_referral_link(context.bot, user_id)
    total = get_referral_count(user_id)
    rewarded = get_referral_rewarded_count(user_id)
    bonus_days = get_referral_bonus_days(user_id)
    tiers = "\n".join(f"• {threshold} рефералов → +{days} д." for threshold, days in REFERRAL_TIERS)
    if not tiers:
        tiers = "• Дополнительные уровни пока не настроены."
    text = (
        "👥 Реферальная система\n\n"
        f"👤 Приглашено — {total}\n"
        f"💳 Оплатили — {rewarded}\n"
        ...
        "🔗 Ваша ссылка:\n"
        f"{link}\n\n"
        f"🎁 За каждого приглашённого, который впервые оплатит подписку, "
        f"вы получаете +{REFERRAL_REWARD_DAYS} день.\n\n"
        "🏆 Уровни:\n"
        f"{tiers}"
    )
    await send_ui(update, context, text, kb_referrals())
...
def find_user_admin(query: str):
    ...
    with db() as conn:
        ...
        return conn.execute(
            "SELECT * FROM profiles WHERE LOWER(username)=LOWER(%s) LIMIT 1",
            (username,),
        ).fetchone()


def admin_user_details(user_id: int) -> tuple[str, InlineKeyboardMarkup]:
    with db() as conn:
        ...
        payments = conn.execute(
            "SELECT COUNT(*) AS total FROM payments WHERE user_id=%s AND status='paid'",
            (user_id,),
        ).fetchone()["total"]
        paid_sum = conn.execute(
            "SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE user_id=%s AND status='paid' AND currency='XTR'",
            (user_id,),
        ).fetchone()["total"]
        crypto_sum = conn.execute(
            "SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE user_id=%s AND status='paid' AND currency=%s",
            (user_id, CRYPTO_ASSET),
        ).fetchone()["total"]
        hugs = conn.execute(
            "SELECT COUNT(*) AS total, COALESCE(SUM(count),0) AS sent FROM hugs WHERE user_id=%s",
            (user_id,),
        ).fetchone()
        searches = conn.execute(
            "SELECT COUNT(*) AS total FROM searches WHERE user_id=%s",
    ...
    if not profile:
        return "❌ Пользователь не найден.", kb_admin()
    sub = get_active_subscription(user_id)
    sub_text = "❌ Нет"
    if sub:
        sub_text = f"✅ {sub['plan']} до {aware(sub['expires_at']).strftime('%d.%m.%Y %H:%M')}"
    bonus = get_referral_bonus_days(user_id)
    text = (
        "👤 ПОЛЬЗОВАТЕЛЬ\n\n"
        f"ID: {profile['user_id']}\n"
        f"Username: @{profile['username']}\n" if profile.get("username") else
        f"ID: {profile['user_id']}\nUsername: —\n"
    )
    text += (
        f"Имя: {(profile.get('first_name') or '')} {(profile.get('last_name') or '')}\n"
        f"Регистрация: {aware(profile['first_seen_at']).strftime('%d.%m.%Y %H:%M') if profile.get('first_seen_at') else '—'}\n\n"
        f"💎 Подписка: {sub_text}\n"
        f"🎁 Ожидающий бонус: {bonus} д.\n\n"
        f"💳 Оплаченных платежей: {payments}\n"
        f"⭐ Stars: {int(paid_sum or 0)}\n"
        f"🪙 {CRYPTO_ASSET}: {float(crypto_sum or 0):g}\n\n"
        f"💬 Сессий sn1c: {hugs['total']}\n"
        f"⭕️ Всего жалоб: {int(hugs['sent'] or 0)}\n"
        f"🔎 Поисков: {searches}\n"
        f"👥 Рефералов: {referrals}\n"
        f"✅ Оплативших рефералов: {rewarded}\n"
        f"📊 Сегодня: {usage_today(user_id)}/{DAILY_REQUEST_LIMIT}"
    )
    markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("💳 Платежи пользователя", callback_data=f"admin:user_payments:{user_id}")],
        [InlineKeyboardButton("📚 Подписки пользователя", callback_data=f"admin:user_subs:{user_id}")],
        [InlineKeyboardButton("⬅️ Админ-панель", callback_data="admin:panel")],
    ])
    return text, markup


async def show_admin_user_payments(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT plan, method, amount, currency, status, created_at
            FROM payments WHERE user_id=%s ORDER BY id DESC LIMIT 15
            """,
            (user_id,),
        ).fetchall()
    lines = [f"💳 ПЛАТЕЖИ {user_id}\n"]
    if not rows:
        lines.append("Платежей нет.")
    else:
        for row in rows:
            status = {"paid": "✅", "pending": "⏳", "failed": "❌", "expired": "⌛"}.get(row["status"], "•")
            amount = row["amount"]
            if amount is None:
                price = "—"
                ...
                price = f"{float(amount):g} {row['currency'] or ''}"
            lines.append(
                f"{status} {row['plan']} | {row['method']} | {price}\n"
                f"   {aware(row['created_at']).strftime('%d.%m.%Y %H:%M')}"
            )
    await send_ui(update, context, "\n".join(lines), kb_admin())


async def show_admin_user_subs(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT plan, method, payment_id, starts_at, expires_at, created_at
            FROM subscriptions WHERE user_id=%s ORDER BY id DESC LIMIT 15
            """,
            (user_id,),
        ).fetchall()
    lines = [f"📚 ПОДПИСКИ {user_id}\n"]
    if not rows:
        lines.append("Подписок нет.")
    else:
        for row in rows:
            lines.append(
                f"• {row['plan']} | {row['method']}\n"
                f"  {aware(row['starts_at']).strftime('%d.%m.%Y %H:%M')} → "
                f"{aware(row['expires_at']).strftime('%d.%m.%Y %H:%M')}\n"
                f"  payment: {row['payment_id'] or '—'}"
            )
    await send_ui(update, context, "\n".join(lines), kb_admin())
...
def admin_stats() -> str:
    ...
    with db() as conn:
        total_users = conn.execute("SELECT COUNT(*) AS total FROM profiles").fetchone()["total"]
        active_subs = conn.execute(
            "SELECT COUNT(DISTINCT user_id) AS total FROM subscriptions WHERE expires_at > NOW()"
        ).fetchone()["total"]
        new_today = conn.execute(
            "SELECT COUNT(*) AS total FROM profiles WHERE first_seen_at::date=%s",
            (today,),
        ).fetchone()["total"]
        paid = conn.execute("SELECT COUNT(*) AS total FROM payments WHERE status='paid'").fetchone()["total"]
        buyers = conn.execute(
            "SELECT COUNT(DISTINCT user_id) AS total FROM payments WHERE status='paid'"
        ).fetchone()["total"]
        stars = conn.execute(
            "SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE status='paid' AND currency='XTR'"
        ).fetchone()["total"]
        crypto = conn.execute(
            "SELECT COALESCE(SUM(amount),0) AS total FROM payments WHERE status='paid' AND currency=%s",
            (CRYPTO_ASSET,),
        ).fetchone()["total"]
        refs = conn.execute("SELECT COUNT(*) AS total FROM referrals").fetchone()["total"]
        converted_refs = conn.execute(
            "SELECT COUNT(*) AS total FROM referrals WHERE rewarded=TRUE"
        ).fetchone()["total"]
        usage = conn.execute(
            "SELECT COALESCE(SUM(requests),0) AS total FROM daily_usage WHERE usage_date=%s",
            (today,),
        ).fetchone()["total"]
        hugs = conn.execute("SELECT COALESCE(SUM(count),0) AS total FROM hugs").fetchone()["total"]
        checks = conn.execute("SELECT COALESCE(SUM(checks),0) AS total FROM profiles").fetchone()["total"]
        searches = conn.execute("SELECT COUNT(*) AS total FROM searches").fetchone()["total"]
        week = conn.execute(
            """
            SELECT COALESCE(SUM(new_users),0) AS users,
                   COALESCE(SUM(requests),0) AS requests,
                   COALESCE(SUM(paid_payments),0) AS paid,
                   COALESCE(SUM(stars_revenue),0) AS stars,
                   COALESCE(SUM(crypto_revenue),0) AS crypto
            FROM daily_stats WHERE stat_date BETWEEN %s AND %s
            """,
            (week_start, today),
        ).fetchone()
        month = conn.execute(
    ...
    return (
        ...
        f"• {CRYPTO_ASSET} — {float(crypto or 0):g}\n\n"
        "🔗 Рефералы\n"
        f"• Всего — {refs}\n"
        f"• Оплативших — {converted_refs}\n\n"
        "📊 Активность\n"
        f"• Запросов сегодня — {usage}/{DAILY_REQUEST_LIMIT}\n"
        f"• Всего sn1cов — {hugs}\n"
        ...
        f"• Оплат — {week['paid']}\n"
        f"• Stars — {int(week['stars'] or 0)}⭐\n"
        f"• {CRYPTO_ASSET} — {float(week['crypto'] or 0):g}\n\n"
        "📅 Текущий месяц\n"
        f"• Новых — {month['users']}\n"
        f"• Запросов — {month['requests']}\n"
        f"• Оплат — {month['paid']}\n"
        f"• Stars — {int(month['stars'] or 0)}⭐\n"
        f"• {CRYPTO_ASSET} — {float(month['crypto'] or 0):g}"
    )
...
async def show_admin_panel(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        await send_ui(update, context, "❌ Доступ запрещён.", kb_home(user_id))
        return
    await send_ui(update, context, admin_stats(), kb_admin())
...
async def show_admin_users(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT user_id, username, first_name, first_seen_at
            FROM profiles ORDER BY first_seen_at DESC LIMIT 20
            """
        ).fetchall()
...
async def show_admin_payments(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT user_id, plan, method, amount, currency, status, created_at
    ...
    if not rows:
        ...
        for row in rows:
            status = {"paid":"✅", "pending":"⏳", "failed":"❌", "expired":"⌛", "timeout":"⌛"}.get(row["status"], "•")
            if row["amount"] is None:
                price = "—"
            elif row["currency"] == "XTR":
                price = f"{int(row['amount'])}⭐"
            else:
                price = f"{float(row['amount']):g} {row['currency'] or ''}"
            lines.append(
                f"{status} {row['user_id']} | {row['plan']} | {row['method']}\n"
                f"   {price} | {aware(row['created_at']).strftime('%d.%m.%Y %H:%M')}"
            )
    await send_ui(update, context, "\n".join(lines), kb_admin())
...
async def show_admin_referrals(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT referrer_id, referred_id, created_at, rewarded
            FROM referrals ORDER BY id DESC LIMIT 20
            """
        ).fetchall()
    ...
    if not rows:
        ...
        for row in rows:
            reward = "✅" if row["rewarded"] else "⏳"
            lines.append(
                f"{reward} {row['referrer_id']} → {row['referred_id']}\n"
                f"   {aware(row['created_at']).strftime('%d.%m.%Y %H:%M')}"
            )
    await send_ui(update, context, "\n".join(lines), kb_admin())


async def show_admin_promos(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = promo_list()
    lines = ["🎟 ПРОМОКОДЫ\n"]
    if not rows:
        ...
        for row in rows:
            limit = "∞" if row["max_uses"] is None else str(row["max_uses"])
            expires = aware(row["expires_at"]).strftime("%d.%m.%Y %H:%M") if row["expires_at"] else "без срока"
            status = "🟢" if row["active"] else "🔴"
            lines.append(
                f"{status} {row['code']} → {row['plan']}\n"
                f"   Использовано: {row['used']}/{limit}\n"
                f"   До: {expires}"
            )
    await send_ui(update, context, "\n".join(lines), kb_admin_promo_list(rows))
...
async def show_admin_health(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        text = (
            ...
            f"👤 Bot — @{bot.username}\n"
            f"📦 DB pool — {DB_POOL.get_stats().get('pool_size', 0)} connections"
        )
        log_event("INFO", "health_check", user_id, {"latency_ms": round(latency, 2)})
    except Exception as exc:
        text = f"❤️ HEALTH\n\n❌ PostgreSQL недоступен.\n{exc}"
        logger.exception("Health check failed")
    await send_ui(update, context, text, kb_admin())
...
def create_backup_bytes() -> tuple[bytes, str]:
    ...
    filename = f"hugbot_backup_{utcnow().strftime('%Y%m%d_%H%M%S')}.json.gz"
    ...
    try:
        ...
        await bot.send_document(
            ...
            caption=(
                "💾 Backup базы готов.\n\n"
                f"Файл: {filename}\n"
                f"Размер: {len(data)/1024:.1f} KB"
            ),
        )
        log_event("INFO", "backup_sent", admin_id, {"filename": filename, "bytes": len(data)})
        return True
    except Exception:
        logger.exception("Backup generation/sending failed")
        log_event("ERROR", "backup_failed", admin_id)
        return False
...
async def issue_channel_invite(bot, user_id: int, plan: str):
    ...
    if not channel:
        logger.error("CHANNEL NOT CONFIGURED: plan=%s user=%s", plan, user_id)
        return None, "no_channel"
    ...
    try:
        ...
        logger.warning(
            "Could not check channel member: plan=%s channel=%s user=%s error=%s",
            plan,
            channel,
            user_id,
            exc,
    ...
    subscription = get_active_subscription(user_id)
    expire_date = aware(subscription["expires_at"]) if subscription else None
    try:
        ...
        return invite.invite_link, "ok"
    except BadRequest as exc:
        logger.error(
            "CHANNEL INVITE FAILED: plan=%s channel=%s user=%s error=%s",
            plan,
            channel,
            user_id,
            exc,
        ...
        logger.error(
            "CHANNEL INVITE TELEGRAM ERROR: plan=%s channel=%s user=%s error=%s",
            plan,
            channel,
            user_id,
            exc,
...
def access_granted_text(plan: str, expires: datetime, invite_status: str):
    ...
    if invite_status == "already_member":
        ...
        text += "\n\nℹ️ Канал подписки не настроен."
    elif invite_status == "error":
        text += f"\n\n⚠️ Не удалось выдать ссылку в канал.\nНапишите {SUPPORT_USERNAME}."
    return text
...
async def grant_paid_access(
    ...
    existing = find_subscription_by_payment(payment_key)
    if existing:
        return aware(existing["expires_at"])

    if payment_db_id:
        payment = get_payment(payment_db_id)
        if not payment or int(payment["user_id"]) != int(user_id) or payment["plan"] != plan:
            raise ValueError("Payment record mismatch")
        update_payment(payment_db_id, "paid", external_id=payment_key)
    ...
    if payment_db_id:
        payment = get_payment(payment_db_id)
        if payment:
            increment_stat("paid_payments")
            if payment["currency"] == "XTR":
                increment_stat("stars_revenue", payment["amount"] or 0)
            elif payment["currency"] == CRYPTO_ASSET:
                increment_stat("crypto_revenue", payment["amount"] or 0)

    await reward_referrer_for_purchase(bot, user_id)
    link, invite_status = await issue_channel_invite(bot, user_id, plan)
    text = access_granted_text(plan, expires, invite_status)
    if link:
        text += "\n\n👇 Ваша ссылка в канал:"
    try:
        await bot.send_message(
        ...
        log_event("ERROR", "paid_access_message_failed", user_id, {"plan": plan})
    return expires
...
async def crypto_api(method: str, payload: dict):
    ...
    if not data.get("ok"):
        raise RuntimeError(
            data.get("error", {}).get("name", "Crypto Pay API error")
        )
    return data["result"]
...
async def create_crypto_invoice(user_id: int, plan: str):
    ...
    if not amount:
        raise RuntimeError(f"USD price is not configured for plan={plan}")
    payload_id = f"hug:{user_id}:{plan}:{int(utcnow().timestamp())}"
...
_crypto_watch_tasks: dict[int, asyncio.Task] = {}
            ...
            try:
                ...
                if items:
                    ...
                    if status in {"expired", "invalid"}:
                        update_payment(payment_db_id, status)
                        return
            except Exception:
                logger.exception("Crypto watcher error invoice=%s", invoice_id)
            await asyncio.sleep(10)
        update_payment(payment_db_id, "timeout")
    finally:
        _crypto_watch_tasks.pop(invoice_id, None)
...
async def start_crypto_watch(
    ...
    existing = _crypto_watch_tasks.get(invoice_id)
    ...
    _crypto_watch_tasks[invoice_id] = task


async def resume_pending_crypto_payments(application: Application):
    if not CRYPTO_PAY_API_TOKEN:
        return
    with db() as conn:
        rows = conn.execute(
            """
            SELECT id, user_id, plan, external_id
            FROM payments
    ...
    for row in rows:
        try:
            invoice_id = int(row["external_id"])
...
async def subscription_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("sub:") and data.count(":") == 1:
        ...
        if plan not in PLANS:
            return
        p = PLANS[plan]
        price_text = f"{p['usd']}$ / {p['stars']}⭐" if p["usd"] else f"{p['stars']}⭐"
        text = (
            f"💎 {p['title']}\n\n"
            f"⏳ Срок — {p['days']} дней\n"
            f"💵 Цена — {price_text}\n\n"
    ...
    if data.startswith("pay:crypto:"):
        ...
        if plan not in PLANS:
            return
        fallback = {
            "week": CRYPTO_FALLBACK_WEEK,
            "month": CRYPTO_FALLBACK_MONTH,
            "year": CRYPTO_FALLBACK_YEAR,
        }.get(plan, "")
        ...
        if not CRYPTO_PAY_API_TOKEN:
            if fallback:
                ...
                await send_ui(
                    update,
                    context,
                    "❌ CryptoBot не настроен.\n\nДобавьте CRYPTO_PAY_API_TOKEN в Render.",
                    kb_pay_methods(plan),
                )
            return

        try:
            invoice, _payload = await create_crypto_invoice(user_id, plan)
            invoice_id = int(invoice["invoice_id"])
            amount = float(PLANS[plan]["usd"])
            ...
            if not url:
                raise RuntimeError("CryptoBot did not return invoice URL")
            markup = InlineKeyboardMarkup([
                [InlineKeyboardButton("💳 Оплатить", url=url)],
                [InlineKeyboardButton(
                    "✅ Я оплатил",
                    callback_data=f"check_crypto:{invoice_id}:{plan}:{payment_db_id}",
                )],
                [InlineKeyboardButton("❌ Отменить", callback_data="pay:cancel")],
            ])
            await send_ui(
    ...
    if data.startswith("pay:stars:"):
        ...
        try:
            await context.bot.send_invoice(
                chat_id=user_id,
                title=p["title"],
                description=f"Доступ к функциям бота на {p['days']} дней.",
            ...
            update_payment(payment_db_id, "failed", failure_reason="invoice_create_failed")
            await context.bot.send_message(
                user_id,
    ...
    if data == "pay:cancel":
        await send_ui(update, context, "❌ Оплата отменена.", kb_home(user_id))
        return
    ...
    if data.startswith("check_crypto:"):
        ...
        try:
            ...
            try:
                await q.answer("Некорректные данные оплаты.", show_alert=True)
            except TelegramError:
                pass
            return
        if plan not in PLANS:
            return
        existing = find_subscription_by_payment(str(invoice_id))
        if existing:
            await send_ui(update, context, "✅ Оплата уже зачислена.", kb_after_pay(None))
            return
        if not CRYPTO_PAY_API_TOKEN:
            try:
                await q.answer("Автопроверка не настроена.", show_alert=True)
            except TelegramError:
                pass
            return
        payment = get_payment(payment_db_id)
        if not payment or int(payment["user_id"]) != user_id or payment["plan"] != plan:
            try:
                await q.answer("Некорректный платёж.", show_alert=True)
            except TelegramError:
                pass
            return
        try:
            ...
            try:
                await q.answer("Оплата ещё не найдена. Попробуйте ещё раз.", show_alert=True)
            except TelegramError:
                pass
        except Exception:
            logger.exception("Crypto manual check failed")
            try:
    ...
    if data.startswith("manual_crypto:"):
        await send_ui(
            update,
            context,
            "⏳ Для автоматической проверки настройте CRYPTO_PAY_API_TOKEN.",
            kb_back_home(),
        )
...
async def pre_checkout(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    parts = payload.split(":")
    valid = False
    error_message = "Счёт недействителен. Откройте оплату заново."

    if len(parts) >= 4 and parts[0] == "hug" and parts[2] in PLANS:
        try:
            user_id = int(parts[1])
            payment_db_id = int(parts[3])
            payment = get_payment(payment_db_id)
            ...
            valid = (
                ...
                and int(payment["user_id"]) == user_id
                and payment["plan"] == plan
                and payment["status"] == "pending"
                and payment["currency"] == "XTR"
                and int(payment["amount"] or 0) == expected
                and int(query.total_amount) == expected
                and query.currency == "XTR"
            )
            if not valid:
                error_message = "Сумма или данные счёта не совпадают. Откройте оплату заново."
        except (ValueError, TypeError):
            valid = False
...
async def successful_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(parts) < 4 or parts[0] != "hug":
        await update.message.reply_text(
            f"❌ Не удалось распознать оплату.\nНапишите {SUPPORT_USERNAME}.",
    ...
    if int(payment.total_amount) != expected:
        if db_id:
            update_payment(db_id, "failed", failure_reason="amount_mismatch")
        log_event("ERROR", "stars_amount_mismatch", user_id, {"expected": expected, "actual": payment.total_amount})
        await update.message.reply_text(
            f"❌ Сумма оплаты не совпала.\nНапишите {SUPPORT_USERNAME}.",
            reply_markup=kb_back_home(),
        )
        return
    ...
    try:
        ...
        context.user_data.pop("pending_star_payment", None)
    except Exception:
        logger.exception("Stars fulfillment failed")
        if db_id:
            update_payment(db_id, "failed", failure_reason="fulfillment_failed")
        await update.message.reply_text(
            f"❌ Оплата получена, но выдача доступа не завершилась.\nНапишите {SUPPORT_USERNAME}.",
            reply_markup=kb_back_home(),
        )
...
async def nav_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "nav:home":
        ...
        await send_ui(update, context, "🎟 Введите промокод:", kb_profile())
...
async def show_admin_daily_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT stat_date, new_users, requests, paid_payments,
                   stars_revenue, crypto_revenue, referrals
            FROM daily_stats
    ...
    if not rows:
        ...
        for row in rows:
            lines.append(
                ...
                f"👥 +{row['new_users']} | 💳 {row['paid_payments']} оплат\n"
                f"📊 {row['requests']} запросов | 🔎 {row['referrals']} рефералов\n"
                f"⭐ {int(row['stars_revenue'] or 0)} | "
                f"🪙 {float(row['crypto_revenue'] or 0):g} {CRYPTO_ASSET}"
            )
    await send_ui(update, context, "\n\n".join(lines), kb_admin())
...
async def show_admin_logs(update: Update, context: ContextTypes.DEFAULT_TYPE):
    with db() as conn:
        rows = conn.execute(
            """
            SELECT level, event, user_id, details, created_at
            FROM app_logs
            ORDER BY id DESC
            LIMIT 25
            """
    ...
    if not rows:
        ...
        for row in rows:
            ...
            details_text = ""
            if details:
                details_text = f" | {json.dumps(details, ensure_ascii=False, default=str)[:180]}"
            lines.append(
                f"[{row['level']}] {row['event']} | {row['user_id'] or '—'}\n"
                f"{aware(row['created_at']).strftime('%d.%m.%Y %H:%M:%S')}{details_text}"
            )
    await send_ui(update, context, "\n\n".join(lines), kb_admin())


async def broadcast_text(bot, text_value: str) -> tuple[int, int]:
    with db() as conn:
        rows = conn.execute("SELECT user_id FROM profiles ORDER BY user_id").fetchall()
    sent = 0
    failed = 0
    for row in rows:
        try:
            await bot.send_message(int(row["user_id"]), text_value)
            sent += 1
        except TelegramError:
            failed += 1
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin:panel":
    ...
    elif data.startswith("admin:user_payments:"):
        try:
            target_id = int(data.rsplit(":", 1)[1])
        except ValueError:
            return
        await show_admin_user_payments(update, context, target_id)
    elif data.startswith("admin:user_subs:"):
        try:
            target_id = int(data.rsplit(":", 1)[1])
        except ValueError:
            return
        await show_admin_user_subs(update, context, target_id)
        ...
        await send_ui(
            ...
            (
                ...
                "Для безлимитных использований: MAX_USES=0\n"
                "Для бессрочного кода вместо даты: -"
            ),
            kb_admin(),
        )
    elif data.startswith("admin:promo_off:"):
        code = data.split(":", 2)[2]
        deactivate_promo(code)
        await show_admin_promos(update, context)
    elif data == "admin:backup":
        await send_ui(update, context, "⏳ Создаю backup базы...", kb_admin())
        await send_backup_to_admin(context.bot, user_id)
        await show_admin_panel(update, context)
    elif data == "admin:health":
        await show_admin_health(update, context)
    elif data == "admin:daily_stats":
        await show_admin_daily_stats(update, context)
        ...
        await send_ui(
            update,
            context,
            "📢 Введите текст рассылки.\n\nПосле отправки бот разошлёт сообщение всем пользователям базы.",
            kb_admin(),
        )
...
async def cmd_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        ...
        return
    await update.message.reply_text(admin_stats(), reply_markup=kb_admin())
...
async def cmd_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        await update.message.reply_text("❌ Доступ запрещён.")
        return
    await update.message.reply_text("⏳ Создаю backup базы...")
    ok = await send_backup_to_admin(context.bot, update.effective_user.id)
    if not ok:
        await update.message.reply_text("❌ Не удалось создать backup. Проверьте логи Render.")
...
async def cmd_user(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update.effective_user.id):
        ...
        return
    if not context.args:
        await update.message.reply_text("Использование: /user 123456789 или /user @username")
        return
    profile = find_user_admin(" ".join(context.args))
    if not profile:
        await update.message.reply_text("❌ Пользователь не найден.")
        return
    text, markup = admin_user_details(int(profile["user_id"]))
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ADMIN STATES
    # ---------------------------------------------
    if is_admin(user_id) and state == "admin_user_search":
        context.user_data["state"] = None
        profile = find_user_admin(text)
        if not profile:
            await update.message.reply_text("❌ Пользователь не найден.", reply_markup=kb_admin())
            return
        details, markup = admin_user_details(int(profile["user_id"]))
        await update.message.reply_text(details, reply_markup=markup)
        return

    if is_admin(user_id) and state == "admin_promo_create":
        context.user_data["state"] = None
        parts = text.split()
        if len(parts) != 4:
            await update.message.reply_text(
                "❌ Формат: CODE PLAN MAX_USES YYYY-MM-DD\nПример: VIP50 month 100 2026-10-31",
        ...
        if not max_raw.isdigit():
            await update.message.reply_text("❌ MAX_USES должен быть числом.", reply_markup=kb_admin())
            return
        max_uses = int(max_raw)
        expires_at = None
        if date_raw != "-":
            try:
                expires_at = datetime.strptime(date_raw, "%Y-%m-%d").replace(
                    hour=23, minute=59, second=59, tzinfo=timezone.utc
                )
            except ValueError:
                await update.message.reply_text("❌ Дата должна быть YYYY-MM-DD.", reply_markup=kb_admin())
                return
        try:
            admin_create_promo(code, plan, max_uses, expires_at)
        except ValueError as exc:
            await update.message.reply_text(f"❌ {exc}", reply_markup=kb_admin())
            return
        await update.message.reply_text(
    ...
    # ADMIN BROADCAST
    # ---------------------------------------------
    if is_admin(user_id) and state == "admin_broadcast":
        context.user_data["state"] = None
        await update.message.reply_text("⏳ Начинаю рассылку...")
        sent, failed = await broadcast_text(context.bot, text)
        log_event("INFO", "broadcast_completed", user_id, {"sent": sent, "failed": failed})
        await update.message.reply_text(
    ...
    if state == "awaiting_hug_target":
        ...
        await update.message.reply_text(
            f"Вы уверены, что хотите отправить жалобы {text}?",
    ...
    if state == "awaiting_confirm":
        await update.message.reply_text("Нажмите кнопку выше 👆", reply_markup=kb_confirm_hug())
        return
    ...
    if state == "awaiting_promo":
        context.user_data["state"] = None
        await apply_promo_text(update, context, user_id, text)
        return
    ...
    if state == "awaiting_check_target":
        context.user_data["state"] = None
        await do_check(update, context, user_id, text)
        return
    ...
    # PROMO WITHOUT STATE
    # ---------------------------------------------
    if text.upper() == PROMO_CODE and PROMO_CODE:
        # Still uses the DB redemption limit/duplicate protection.
        await apply_promo_text(update, context, user_id, text)
        return
...
async def remove_from_channels(bot, user_id: int, expires_at: datetime):
    ...
    for channel in channels:
        with db() as conn:
            already = conn.execute(
                """
                SELECT 1 FROM channel_revocations
                WHERE user_id=%s AND channel=%s AND revoked_for_expires_at=%s
                LIMIT 1
                """,
        ...
        try:
            ...
            logger.warning(
                "Could not remove expired user=%s from channel=%s: %s",
                user_id,
                channel,
                exc,
            )
        with db() as conn:
            conn.execute(
                """
                INSERT INTO channel_revocations(user_id,channel,revoked_for_expires_at,revoked_at)
                VALUES(%s,%s,%s,%s)
                ON CONFLICT DO NOTHING
                """,
                (user_id, channel, expires_at, utcnow()),
            )
            conn.commit()
    try:
        await bot.send_message(
...
async def cleanup_loop(application: Application):
    while True:
        try:
            with db() as conn:
                conn.execute(
                    "DELETE FROM app_logs WHERE created_at < NOW() - (%s * INTERVAL '1 day')",
                    (LOG_RETENTION_DAYS,),
                )
                conn.execute(
                    "DELETE FROM daily_usage WHERE usage_date < CURRENT_DATE - (%s * INTERVAL '1 day')",
                    (USAGE_RETENTION_DAYS,),
                )
                conn.execute(
                    "DELETE FROM daily_stats WHERE stat_date < CURRENT_DATE - (%s * INTERVAL '1 day')",
                    (STATS_RETENTION_DAYS,),
                )
                conn.commit()
            log_event("INFO", "cleanup_completed")
        except asyncio.CancelledError:
            raise
        except Exception:
...
async def auto_backup_loop(application: Application):
    if not AUTO_BACKUP or not AUTO_BACKUP_ADMIN_ID:
        return
    while True:
        try:
            now = utcnow()
            key = now.strftime("%Y-%m-%d-%H")
            if now.hour == AUTO_BACKUP_HOUR_UTC and now.minute < 5:
                with db() as conn:
                    row = conn.execute(
                        "SELECT value FROM meta WHERE key='last_auto_backup_hour'"
                    ).fetchone()
                    last = row["value"] if row else ""
                if last != key:
                    ok = await send_backup_to_admin(application.bot, AUTO_BACKUP_ADMIN_ID)
                    if ok:
                        with db() as conn:
                            conn.execute(
                                """
                                INSERT INTO meta(key,value) VALUES('last_auto_backup_hour',%s)
            ...
            logger.exception("Auto backup loop failed")
...
async def post_init(application: Application):
    ...
    # Reattach pending CryptoBot invoices after a Render restart.
    await resume_pending_crypto_payments(application)
...
async def post_shutdown(application: Application):
    global DB_OPENED
    try:
        if DB_OPENED and not DB_POOL.closed:
            DB_POOL.close()
            DB_OPENED = False
    except Exception:
...
def main():
    ...
    logger.info(
        "Config: daily_limit=%s referral_reward=%s tiers=%s auto_backup=%s",
        DAILY_REQUEST_LIMIT,
